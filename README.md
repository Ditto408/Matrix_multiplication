...
| Dense    | 1000  | 1.0   | 377.5097 | –            | 5.3989       | 8.253       |
| Dense    | 2000  | 1.0   | –        | –            | 15.9631      | 15.560      |
...
| Dense    | 1000  | 1.0   | 597.5 MB/597.5 MB | –                 | 44.2 MB/44.2 MB   | 83.8 MB/69.9 MB   |
| Dense    | 2000  | 1.0   | –                 | –                 | 210.8 MB/210.8 MB | 361.3 MB/361.3 MB |
...
未来可以考虑使用 Spark 原生的 Scala 实现，或结合 JNI、C++ 等方式进一步减少 Python 层带来的运行时开销，从而提升分块矩阵乘法在超大规模场景下的性能表现。
...
本项目由四名小组成员协作完成，共同完成了分布式矩阵乘法的实现、实验与分析工作。具体分工如下：

林宇轩：
组长。负责与协调实验总体规划与分工、进度控制和组织讨论，参与优化方案讨论，收集和整理实验日志与性能数据，实验结果的整理与可视化分析，实验结论的总结与项目文档的撰写，答辩PPT制作，答辩视频的录制与剪辑。

李浚宏：
实验环境与结点搭建，spark服务器的维护，实验方案详细设计，SystemML相关实验的配置与执行，参与优化方案讨论。

钱伟鹏：
Spark分布式矩阵乘法的实现与优化，不同规模与稀疏度矩阵的实验配置与运行。

张航：
矩阵数据生成模块的实现，朴素算法的实现，参与优化方案讨论，实验结果的整理与可视化分析，实验结论的总结与项目文档的撰写，答辩PPT制作。
